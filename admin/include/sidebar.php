<div class="span3">
					<div class="sidebar">
						<ul class="widget widget-menu unstyled">
							<li>
								<a class="collapsed" data-toggle="collapse" href="#togglePages">
									<i class="menu-icon icon-cog"></i>
									<i class="icon-chevron-down pull-right"></i><i class="icon-chevron-up pull-right"></i>
									Manage Complaint
								</a>
								<ul id="togglePages" class="collapse unstyled">
									<li>
										<a href="notprocess-complaint.php">
											<i class="icon-tasks"></i>
											Not Process Yet Complaint
											<?php
											$department = $_SESSION['department'];
											$rights = $_SESSION['rights'];
											if($rights == '0'){
												$rt = mysqli_query($con,'SELECT * FROM tblcomplaints where status is null');
											}else{
												$rt = mysqli_query($con,'SELECT * FROM tblcomplaints where status is null and department="'.$department.'"');
											}
											$num1 = mysqli_num_rows($rt);
											{?>
											<b class="label orange pull-right"><?php echo htmlentities($num1); ?></b>
											<?php } ?>
										</a>
									</li>
									<li>
										<a href="inprocess-complaint.php">
											<i class="icon-tasks"></i>
											Pending Complaint
                   							<?php 
											  $status="in Process";
											  $department = $_SESSION['department'];
											  $rights = $_SESSION['rights'];
											  if($rights == '0'){
												$rt = mysqli_query($con,"SELECT * FROM tblcomplaints where status='$status'");
											  }else{
												$rt = mysqli_query($con,"SELECT * FROM tblcomplaints where status='$status' and department='$department'");
											  }
											
											$num1 = mysqli_num_rows($rt);
											{?><b class="label orange pull-right"><?php echo htmlentities($num1); ?></b>
											<?php } ?>
										</a>
									</li>
									<li>
										<a href="closed-complaint.php">
											<i class="icon-inbox"></i>
											Closed Complaints
	     									<?php 
											  $status="closed";          
											  $department = $_SESSION['department'];
											  $rights = $_SESSION['rights'];
											  if($rights == '0'){
												$rt = mysqli_query($con,"SELECT * FROM tblcomplaints where status='$status'");
											  }else{
												$rt = mysqli_query($con,"SELECT * FROM tblcomplaints where status='$status' and department='$department'");
											  }

											
											$num1 = mysqli_num_rows($rt);
											{?><b class="label green pull-right"><?php echo htmlentities($num1); ?></b>
											<?php } ?>

										</a>
									</li>
								</ul>
							</li>
							<?php 
							if($_SESSION['rights']=='0'){
								echo '
								<li>
								<a href="manage-users.php">
									<i class="menu-icon icon-group"></i>
									Manage Students
								</a>
							</li>
							<li>
								<a href="manage-admins.php">
									<i class="menu-icon icon-group"></i>
									Manage Admins
								</a>
							</li>
							<li>
								<a href="manage-department.php">
									<i class="menu-icon icon-paste"></i>
									Manage Departments
								</a>
							</li>
								';
							}
							?>
							
							
						</ul>
						<?php 
							if($_SESSION['rights']=='0'){
								echo '
								<ul class="widget widget-menu unstyled">
                                <li><a href="../users/registration.php"><i class="menu-icon icon-user"></i> Add Student </a></li>
                                <li><a href="../users/addSubAdmin.php"><i class="menu-icon icon-user"></i> Add Sub-Admin </a></li>
                                <li><a href="../users/addDepartment.php"><i class="menu-icon icon-paste"></i>Add Department</a></li>                        
                            </ul>

						<ul class="widget widget-menu unstyled">
                                <li><a href="category.php"><i class="menu-icon icon-tasks"></i> Add Category </a></li>
                                <li><a href="subcategory.php"><i class="menu-icon icon-tasks"></i>Add Sub-Category </a></li>
                                <li><a href="state.php"><i class="menu-icon icon-paste"></i>Add State</a></li>
                          
								<li><a href="user-logs.php"><i class="menu-icon icon-tasks"></i>User Login Log </a></li>
                            </ul><!--/.widget-nav-->
								';
							}
						?>
						

						<ul class="widget widget-menu unstyled">
							<li>
								<a href="logout.php">
									<i class="menu-icon icon-signout"></i>
									Logout
								</a>
							</li>
						</ul>

					</div><!--/.sidebar-->
				</div><!--/.span3-->
