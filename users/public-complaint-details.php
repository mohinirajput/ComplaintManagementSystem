...
<?php include('includes/sidebar.php');?>
 ...
 <?php $query=mysqli_query($con,"select tblcomplaints.*,category.categoryName as catname from tblcomplaints join category on category.id=tblcomplaints.category where complaintNumber='".$_GET['cid']."'");
  
  
while($row=mysqli_fetch_array($query)){
  // echo $row['userId'];
  $userQuery = mysqli_query($con,'select * from users where id="'.$row["userId"].'"');
  $fetchuser = mysqli_fetch_array($userQuery);
  // echo mysqli_num_rows($userQuery);
  ?>
          	<div class="row mt">
...
<label class="col-sm-2 col-sm-2 control-label"><b>Reg. Date :</b></label>
            ...
            <label class="col-sm-2 col-sm-2 control-label"><b>Name : </b></label>
          		<div class="col-sm-4">
          		<p><?php echo htmlentities($fetchuser['fullName']);?></p>
          		</div>
              <label class="col-sm-2 col-sm-2 control-label"><b>Email :</b></label>
              <div class="col-sm-4">
              <p><?php echo htmlentities($fetchuser['userEmail']);?></p>
              </div>
          	</div>
